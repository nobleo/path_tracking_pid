...
bool TrackingPidLocalPlanner::computeVelocityCommands(geometry_msgs::Twist & cmd_vel)
{
  ...
  if (pid_controller_.getConfig().anti_collision) {
    ...
    if (cost >= costmap_2d::INSCRIBED_INFLATED_OBSTACLE) {
      pid_controller_.setVelMaxObstacle(units::Speed{0});
      ...
      pid_controller_.setVelMaxObstacle(units::Speed{limit});
    } else {
      pid_controller_.setVelMaxObstacle(units::Speed{INFINITY});  // set back to inf
    }
  } else {
    pid_controller_.setVelMaxObstacle(
      units::Speed{INFINITY});  // Can be disabled live, so set back to inf
  }

  path_tracking_pid::PidDebug pid_debug;
  auto eda = units::Duration{
    1 /
    FLT_EPSILON};  // initial guess. Avoids errors in case function returns due to wrong delta_t;
  double progress = 0.0;
  ...
  geometry_msgs::TwistStamped & cmd_vel, std::string & /* message */)
{
  ...
  bool moving = abs(units::Speed{cmd_vel.twist.linear.x}) > VELOCITY_EPS;
...
{
  pid_controller_.setVelMaxExternal(units::Speed{msg.data});
}
}  // namespace path_tracking_pid
